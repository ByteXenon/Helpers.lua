--[[
  ...
  This library provides various helper functions for common tasks in Lua programming.
]]

--* Store standard library functions in variables for performance *--
...
function Helpers.StringifyTable(Table, Spacing)
	...
	function VisitTableElement(Lines, Indentation, Index, Value)
...
--function Helpers.StringFormat(String, Env, ...)
...
-- Combines multiple tables and returns a function that retrieves a value based on the given key.
-- If the value is not found in the first table, it searches the subsequent tables.
-- Parameters:
--   ...: The tables to merge and retrieve values from.
-- Returns:
--   A function that takes a key and returns the corresponding value from the merged tables.
...
function Helpers.NewClass(baseClass)
	local function classConstructor(self, ...)
		...
		-- ClassProperties are
		sharedObjects = {classProperties, baseClass, rawSuperClass}
